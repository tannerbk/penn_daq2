...
int ChinjScan(int crateNum, uint32_t slotMask, uint32_t channelMask, float frequency, int gtDelay, int pedWidth, int numPedestals, float upper, float lower, int pedOn, int quickOn, int updateDB, int finalTest)
{
  ...
  int chinj_err[16];
  float pedestal_qhl[16][32]={{0}};
  float pedestal_qhs[16][32]={{0}};
  float pedestal_qlx[16][32]={{0}};
        ...
        if ((0x1<<slot_iter) & slotMask){
          ...
          for (int i = 0; i<32; i++){
            ...
            for (int j=0;j<16;j++){
              ...
              if(j ==0){
                if (dacvalue == 0){
                  pedestal_qhl[slot_iter][i] = ped[i].thiscell[j].qhlbar;
                  pedestal_qhs[slot_iter][i] = ped[i].thiscell[j].qhsbar;
                  pedestal_qlx[slot_iter][i] = ped[i].thiscell[j].qlxbar;
                }
                if (dacvalue == 250){ // Only the large DAC value
                  // Checks whether integrator is getting pmt input
                  float qhl_difference = abs(ped[i].thiscell[j].qhlbar - pedestal_qhl[slot_iter][i]);
                  float qhs_difference = abs(ped[i].thiscell[j].qhsbar - pedestal_qhs[slot_iter][i]);
                  float qlx_difference = abs(ped[i].thiscell[j].qlxbar - pedestal_qlx[slot_iter][i]);
